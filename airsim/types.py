...
class MsgpackMixin:
    ...
    def to_msgpack(self, *args, **kwargs):
        d = self.__dict__
        return list(v.to_msgpack() if hasattr(v, 'to_msgpack') else v for v in d.values())
        ...
        if isinstance(encoded, dict):
            ...
            for i, k in enumerate(keys):
                if i < len(encoded):
                    ...
                    if isinstance(v, (list, tuple)) and hasattr(attr, 'from_msgpack'):
                        setattr(obj, k, attr.__class__.from_msgpack(v))
                    else:
                        setattr(obj, k, v)
        return obj
...
class Quaternionr(MsgpackMixin):
    ...
    def __init__(self, x_val = 0.0, y_val = 0.0, z_val = 0.0, w_val = 1.0):
        # NOTE: Assignment order must match C++ MSGPACK_DEFINE_ARRAY(w_val, x_val, y_val, z_val)
        # because MsgpackMixin.to_msgpack() iterates self.__dict__ in insertion order.
        self.w_val = w_val
        self.x_val = x_val
        self.y_val = y_val
        self.z_val = z_val
